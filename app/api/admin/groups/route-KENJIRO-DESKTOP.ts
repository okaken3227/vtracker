...
export async function PATCH(req: NextRequest) {
  const { id, icon_url, keywords, parent_group_id, name, color, category, slug } = await req.json();
  ...
  if (slug !== undefined) updates.slug = slug ? slug.trim().toLowerCase().replace(/\s+/g, "_") : null;
