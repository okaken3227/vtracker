...
function PeakStar({ cx, cy, fill }: { cx?: number; cy?: number; fill: string }) {
  if (cx == null || cy == null) return null;
  // 正五芒星（外径9・内径3.5）― 上向き、(0,0)中心
  const pts = "0,-9 2.06,-2.83 8.56,-2.78 3.33,1.08 5.29,7.28 0,3.5 -5.29,7.28 -3.33,1.08 -8.56,-2.78 -2.06,-2.83";
  return (
        ...
        .map((entry) => {
          ...
          return (
            <div key={entry.dataKey} className="flex items-center gap-2 py-0.5">
              ...
              {isPeak && <span className="text-xs text-amber-400">★</span>}
        ...
        >
          ...
          {visibleStreams.map((s) => (
            <Line
              ...
              stroke={s.color}
              strokeWidth={2.5}
              dot={(props: { cx?: number; cy?: number; payload?: { t: number } }) => {
                const isPeak = props.payload?.t === s.peakT;
                if (!isPeak) return <g key={`dot-${props.cx}-${props.cy}`} />;
                return <PeakStar key={`star-${s.videoId}-${props.cx}`} cx={props.cx} cy={props.cy} fill={s.color} />;
              }}
              activeDot={(props: { cx?: number; cy?: number; payload?: { t: number } }) => {
                const isPeak = props.payload?.t === s.peakT;
                if (isPeak) {
                  return <PeakStar key={`astar-${s.videoId}-${props.cx}`} cx={props.cx} cy={props.cy} fill={s.color} />;
                }
                return (
                  <circle
                    key={`adot-${s.videoId}-${props.cx}`}
                    cx={props.cx}
                    cy={props.cy}
                    r={5}
                    fill={s.color}
