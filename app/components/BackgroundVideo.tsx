...
const btnClass =
  "pointer-events-auto flex items-center gap-1 rounded-full bg-black/20 px-2.5 py-1 text-[10px] font-medium text-white/60 backdrop-blur-sm transition-all hover:bg-black/40 hover:text-white";
...
export default function BackgroundVideo({ videoIds }: { videoIds: string[] }) {
  ...
  const buttons = (
    <>
      ...
      {enabled && videoIds.length > 1 && (
        <button onClick={shuffle} className={btnClass}>
          <svg className="h-2.5 w-2.5 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <>
      ...
      {/* ボタン: PCはヘッダーエリア右、モバイルはボトム右 */}
      {createPortal(
        <>
          {/* PC: ヘッダーエリア */}
          <div
            className="pointer-events-none fixed inset-x-0 top-0 hidden h-14 items-center justify-end gap-1.5 px-4 lg:flex"
            style={{ zIndex: 31 }}
          >
            {buttons}
          </div>
          {/* モバイル: ボトム右 */}
          <div
            className="pointer-events-none fixed bottom-4 right-4 flex items-center gap-1.5 lg:hidden"
            style={{ zIndex: 31 }}
          >
            {buttons}
          </div>
